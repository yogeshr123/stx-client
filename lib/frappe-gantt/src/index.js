...
export default class Gantt {
    ...
    setup_gantt_dates() {
        ...
        const tomorrow = new Date(this.gantt_start);
        tomorrow.setDate(tomorrow.getDate() + 1);

        this.gantt_start = date_utils.start_of(this.gantt_start, 'day');
        this.gantt_end = date_utils.start_of(tomorrow, 'day');
        ...
        for (let date of this.get_dates_to_draw()) {
            ...
            if (date.upper_text) {
                const $upper_text = createSVG('text', {
                    x: date.upper_x + 80,
                    y: date.upper_y,
                    innerHTML: (new Date().getMonth() + 1) + ' / ' + date.upper_text + ' / ' + new Date().getFullYear(),
                    class: 'upper-text',
                    append_to: this.layers.date
                });
    ...
    get_date_info(date, last_date, i) {
        if (!last_date) {
            ...
            last_date = date_utils.add(yesterday, 1, 'year');
        }
        const date_text = {
