...
export class AddLoadControlComponent implements OnInit {
    ...
    getUserInfo() {
        if (
            ...
            this.addLoadControlForm.controls.UPDATED_BY.patchValue(
                this.appState.loggedInUser.USER_NAME
            );
        }
    }
    ...
    getEmails() {
        this.loadControlService.getEmails().subscribe((data: any) => {
            if (data.data && data.data.length > 0) {
                ...
                this.addLoadControlForm.controls.EMAIL_GROUP.patchValue(
    ...
    factENVSelected() {
        const factSchemas = this.factSchemaNamesBackup.filter(
            i => i.ENV_NAME === this.addLoadControlForm.value.FACT_ENV_NAME
        );
    ...
    factSchemaSelected() {
        const factTables = this.factSchemaNamesBackup.filter(
            i =>
                i.SCHEMA_NAME ===
                    this.addLoadControlForm.value.FACT_SCHEMA_NAME &&
    ...
    etlStatusChanged() {
        if (this.addLoadControlForm.value.ETL_STATUS !== 'HOLD') {
            ...
            this.addLoadControlForm.controls.ETL_STATUS_REASON.setValidators(
                null
            );
    ...
    setTableSourceValidators() {
        ...
        const DB_TABLE_PK_COLUMNS_SCHEMA = this.addLoadControlForm.get(
            'DB_TABLE_PK_COLUMNS'
        );
        const DB_TABLE_UPDATE_DATE_COLUMN = this.addLoadControlForm.get(
            'DB_TABLE_UPDATE_DATE_COLUMN'
        );
        const CHECK_INDEX_EXIST = this.addLoadControlForm.get(
            'CHECK_INDEX_EXIST'
        );
        const RAW_FACTORY_PATH = this.addLoadControlForm.get(
            'RAW_FACTORY_PATH'
        );
        const RAW_FACTORY_RETENTION_STRATEGY = this.addLoadControlForm.get(
            'RAW_FACTORY_RETENTION_STRATEGY'
        );
        const RAW_FACTORY_RETENTION_DAYS = this.addLoadControlForm.get(
            'RAW_FACTORY_RETENTION_DAYS'
        );
        const RAW_FACTORY_MAX_LANDING_DATE = this.addLoadControlForm.get(
            'RAW_FACTORY_MAX_LANDING_DATE'
        );
        const RAW_FACTORY_SYNC_STATUS = this.addLoadControlForm.get(
            'RAW_FACTORY_SYNC_STATUS'
        );
        const EMAIL_GROUP = this.addLoadControlForm.get('EMAIL_GROUP');

        this.addLoadControlForm
            .get('TABLE_SOURCE')
    ...
    setRetentionStrategyValidators() {
        ...
        this.addLoadControlForm
            ...
            .valueChanges.subscribe(T1_RETENTION_STRATEGY => {
                const T1_RETENTION_DAYS = this.addLoadControlForm.get(
                    'T1_RETENTION_DAYS'
                );
        ...
        this.addLoadControlForm
            ...
            .valueChanges.subscribe(T2_T3_RETENTION_STRATEGY => {
                const T2_T3_RETENTION_DAYS = this.addLoadControlForm.get(
    ...
    setSchemaAndTableValues() {
        const TARGET_SCHEMA_NAME = this.addLoadControlForm.get(
            'TARGET_SCHEMA_NAME'
        );
        const TARGET_TABLE_NAME = this.addLoadControlForm.get(
            'TARGET_TABLE_NAME'
        );
        const DB_SCHEMA = this.addLoadControlForm.get('DB_SCHEMA');
        const DB_TABLE = this.addLoadControlForm.get('DB_TABLE');
        ...
        this.addLoadControlForm
            .get('SCHEMA_NAME')
            .valueChanges.subscribe(SCHEMA_NAME => {
                if (
                    TARGET_SCHEMA_NAME.value.trim() === '' ||
                    TARGET_SCHEMA_NAME.untouched
                ) {
        ...
        this.addLoadControlForm
            .get('TABLE_NAME')
            .valueChanges.subscribe(TABLE_NAME => {
                if (
                    TARGET_TABLE_NAME.value.trim() === '' ||
                    TARGET_TABLE_NAME.untouched
                ) {
    ...
    setLoadStrategyValidators() {
        ...
        const DB_TABLE_PK_COLUMNS = this.addLoadControlForm.get(
            'DB_TABLE_PK_COLUMNS'
        );
        const CHECK_INDEX_EXIST = this.addLoadControlForm.get(
            'CHECK_INDEX_EXIST'
        );
        const T1_MAX_LOAD_END_DATE = this.addLoadControlForm.get(
            'T1_MAX_LOAD_END_DATE'
        );
        const T1_CLUSTER_ID = this.addLoadControlForm.get('T1_CLUSTER_ID');
        const T2_INSERT_DIR_BATCH_SIZE = this.addLoadControlForm.get(
            'T2_INSERT_DIR_BATCH_SIZE'
        );
        const T2_INSERT_BATCH_FILE_SIZE_GB = this.addLoadControlForm.get(
            'T2_INSERT_BATCH_FILE_SIZE_GB'
        );
        const T1_BATCH_IN_DAYS = this.addLoadControlForm.get(
            'T1_BATCH_IN_DAYS'
        );
        const T1_SPARK_CONFIG_ID = this.addLoadControlForm.get(
            'T1_SPARK_CONFIG_ID'
        );
        const DB_ID = this.addLoadControlForm.get('DB_ID');
        const DB_SCHEMA = this.addLoadControlForm.get('DB_SCHEMA');
        ...
        const FACT_SCHEMA_NAME = this.addLoadControlForm.get(
            'FACT_SCHEMA_NAME'
        );
    ...
    onSubmit() {
        ...
        this.loadControlService.addRecord(body).subscribe(
            (data: any) => {
