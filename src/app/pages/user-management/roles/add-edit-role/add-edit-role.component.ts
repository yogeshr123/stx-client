...
export class AddEditRoleComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        if (this.routeInfo.isEditMode || this.routeInfo.isViewOnly) {
            ...
            this.selectedRole.PERMISSIONSARRAY = this.selectedRole.PERMISSIONS.split(
                ','
            ).map(Number);
        } else {
            this.selectedRole = {};
    ...
    loadCurrentRolesPermissions() {
        ...
        mainPermissions.forEach((element: Permission) => {
            ...
            if (this.routeInfo.isEditMode || this.routeInfo.isViewOnly) {
                hasUserPermission = this.selectedRole.PERMISSIONSARRAY.some(
                    t => t === element.ID
                );
            }
            ...
            children.forEach(child => {
                ...
                if (this.routeInfo.isEditMode || this.routeInfo.isViewOnly) {
                    hasUserChildPermission = this.selectedRole.PERMISSIONSARRAY.some(
                        t => t === child.ID
                    );
                }
    ...
    isSelectedChanged($event, permission: Permission) {
        if (permission.children.length === 0 && permission.isSelected) {
            const root = find(
                ...
                (item: Permission) => item.ID === permission.PARENT
            );
                ...
                (data: any) => {
                    ...
                    if (editedRole.ID === this.appState.loggedInUser.ROLE) {
                        this.confirmationService.confirm({
                            ...
                            acceptLabel: 'Ok',
                            message:
                                'You have changed the permissions of logged in user. Please login again...',
                            accept: () => {
