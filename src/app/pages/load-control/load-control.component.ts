...
export class LoadControlComponent implements OnInit {
            ...
            (data: any) => {
                if (data.data && data.data.length > 0) {
                    ...
                    for (const key in this.recordsArray[0]) {
                        if (this.recordsArray[0].hasOwnProperty(key)) {
    ...
    changeETLStatus(status: string) {
        if (this.selectedRecords.length > 0) {
            ...
            for (const iterator of this.selectedRecords) {
                ...
                if (status === 'TRIGGER') {
                    if (
                        (iterator.TABLE_STATUS === 'TODO' ||
                            iterator.TABLE_STATUS === 'COMPLETE') &&
                        iterator.DAG_AVAILABLE_STATUS === 'COMPLETE'
                    ) {
                        records.push({
            ...
            if (records.length > 0) {
                ...
                if (status === 'KILL') {
                    this.showToast(
                        'error',
                        'Kill DAG functionality is not implemented!'
                    );
                }
                ...
                this.showToast(
                    'info',
                    'No record found for the valid criteria'
                );
            }
        }
    }
        ...
        if (this.selectedRecords.length > 0) {
            ...
            this.loadControlService.changeStatus(body).subscribe(
                (data: any) => {
                    ...
                    this.showToast(
                        'success',
                        `${this.statusType} status changed`
                    );
        ...
        if (this.selectedRecords.length > 0) {
            ...
            this.loadControlService.resetExecutionStatus(body).subscribe(
                (data: any) => {
                    ...
                    this.showToast(
                        'success',
                        `${status} Execution status changed`
                    );
                },
                error => {
                    this.showToast(
                        'error',
                        'Could not update execution status.'
                    );
                }
            );
        }
    ...
    search() {
        if (
            ...
            this.loadControlService.getSearchQueryResult(body).subscribe(
                (data: any) => {
                    if (data && data.data) {
                        this.recordsArray = data.data;
                        this.checkSideArrows();
    ...
    onCheckWeeklyRecurrencePattern(event, inputId) {
        if (event) {
            ...
            this.weekelyRecurrenceCheckedDays = this.weekelyRecurrenceCheckedDays.filter(
                i => i !== inputId
            );
        }
    }
    ...
    onSchedulerSubmit() {
        ...
        if (this.recurrencePatterIndex === 0) {
            ...
            const hourlyRecurrencePatternMinute = this.schedulerForm.controls
                .hourlyRecurrencePatternMinute.value;
            if (
                hourlyRecurrencePatternMinute >= 0 &&
                hourlyRecurrencePatternMinute <= 59
            ) {
            ...
            const scheduleStartTime = this.schedulerForm.controls
                .scheduleStartTime.value;
            const scheduleStartTimeHour = new Date(
                scheduleStartTime
            ).getHours();
            const scheduleStartTimeMin = new Date(
                scheduleStartTime
            ).getMinutes();
            //  Set daily recucrrence pattern
            if (
                this.schedulerForm.controls.dailyRecurrencePattern.value ===
                'selectedday'
            ) {
                const dailyRecurrencePatternDay = this.schedulerForm.controls
                    .dailyRecurrencePatternDay.value;
            ...
            if (
                this.schedulerForm.controls.monthlyRecurrencePattern.value ===
                'selectedmonthday'
            ) {
                const monthlyRecurrencePatternDay = this.schedulerForm.controls
                    .monthlyRecurrencePatternDay.value;
                const monthlyRecurrencePatternMonth = this.schedulerForm
                    .controls.monthlyRecurrencePatternMonth.value;
                if (
                    monthlyRecurrencePatternDay > 0 &&
                    monthlyRecurrencePatternDay <= 31 &&
                    monthlyRecurrencePatternMonth > 0 &&
                    monthlyRecurrencePatternMonth <= 12
                ) {
                ...
                const monthlyRecurrencePatternDayFormat = this.schedulerForm
                    .controls.monthlyRecurrencePatternDayFormat.value;
                const monthlyRecurrencePatternWeekFormat = this.schedulerForm
                    .controls.monthlyRecurrencePatternWeekFormat.value;
                const monthlyRecurrencePatternMonth = this.schedulerForm
                    .controls.monthlyRecurrencePatternMonth.value;
                if (
                    monthlyRecurrencePatternMonth > 0 &&
            ...
            if (
                this.schedulerForm.controls.yearlyRecurrencePattern.value ===
                'selectedyearmonth'
            ) {
                const yearlyRecurrencePatternMonth = this.schedulerForm.controls
                    .yearlyRecurrencePatternMonth.value;
                const yearlyRecurrencePatternDay = this.schedulerForm.controls
                    .yearlyRecurrencePatternDay.value;
                if (
                    yearlyRecurrencePatternDay > 0 &&
                    yearlyRecurrencePatternDay <= 31
                ) {
                    // tslint:disable-next-line:max-line-length
                ...
                const yearlyRecurrencePatternDayFormat = this.schedulerForm
                    .controls.yearlyRecurrencePatternDayFormat.value;
                const yearlyRecurrencePatternWeekFormat = this.schedulerForm
                    .controls.yearlyRecurrencePatternWeekFormat.value;
                const yearlyRecurrencePatternMonth = this.schedulerForm.controls
                    .yearlyRecurrencePatternMonth.value;
                // tslint:disable-next-line:max-line-length
            ...
            if (isValidCronExpression) {
                ...
                this.loadControlService.setSchedulerInterval(body).subscribe(
                    ...
                    error => {
                        this.showToast(
                            'error',
                            'Could not save Scheduler interval.'
                        );
                    }
                );
            }
