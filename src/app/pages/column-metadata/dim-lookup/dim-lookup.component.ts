...
export class DimLookupComponent implements OnInit {
    ...
    allColumns = [];
    deleteError = '';
    metadataVersionsList = [];
    selectedMetadataVersion: any;
    ...
    ngOnInit() {
        ...
        if (this.state.CMV && this.state.CMV.selectedTable) {
            this.metadataVersionsList = [];
            this.selectedTable = this.state.CMV.selectedTable;
            this.selectedMetadataVersion = this.selectedTable.METADATA_VERSION;
            for (let i = 1; i <= this.selectedTable.METADATA_VERSION; i++) {
                this.metadataVersionsList.push(i);
            }
        }
    ...
    getLookUps() {
        ...
        const request = {
            ...
            columnVersion: this.selectedMetadataVersion,
        };
        this.columnMetadataService.getTableLookUps(request).subscribe(
            (resp: any) => {
    ...
    getRefreshTables() {
        this.columnMetadataService.getRefreshTables().subscribe(
            (resp: any) => {
                        ...
                        if (!resp.error && resp.data && resp.data.length) {
                            const selectedColumns = resp.data.filter(
                                i =>
                                    ...
                                    lookUp.LOOKUP_TABLE_ALIAS
                            );
                            if (selectedColumns && selectedColumns.length) {
                                this.deleteError =
                                    'You must delete all the added columns first.';
                            } else {
                                this.columnMetadataService
                                    .deleteLookups({ data: lookUp })
                                    .subscribe(
                                        (resp2: any) => {
                                            if (!resp2.error) {
                                                this.showToast(
