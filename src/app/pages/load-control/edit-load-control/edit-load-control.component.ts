...
export class EditLoadControlComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (
            this.appState.selectedRecord &&
            this.appState.selectedRecord.record
        ) {
    ...
    getUserInfo() {
        if (
            ...
            this.editLoadControlForm.controls.UPDATED_BY.patchValue(
                this.appState.loggedInUser.USER_NAME
            );
        }
    }
    ...
    formInit() {
        this.editLoadControlForm = this.formBuilder.group({
            ...
            TARGET_SCHEMA_NAME: [
                ...
                Validators.compose([
                    Validators.required,
                    Validators.pattern(this.tableRegex),
                ]),
            ],
            TARGET_TABLE_NAME: [
                '',
    ...
    get f() {
        return this.editLoadControlForm.controls;
    }
        ...
        this.loadControlService.getEmails().subscribe((data: any) => {
            if (data.data && data.data.length > 0) {
                ...
                if (!this.editLoadControlForm.controls.EMAIL_GROUP.value) {
                    this.editLoadControlForm.controls.EMAIL_GROUP.patchValue(
                        this.emails[0].EMAIL_GROUP
                    );
                }
            }
        });
        ...
        this.loadControlService.getRecords().subscribe(
            (data: any) => {
                if (data.data && data.data.length > 0) {
                    ...
                    this.factSchemaNamesBackup = factSchemaNames.filter(
                        i =>
                            i.LOAD_STRATEGY === 'INSERT' ||
                            i.LOAD_STRATEGY === 'UPDATE'
                    );
    ...
    factENVSelected() {
        const factSchemas = this.factSchemaNamesBackup.filter(
            i => i.ENV_NAME === this.editLoadControlForm.value.FACT_ENV_NAME
        );
        this.factSchemaNames = this.removeDuplicates(
            factSchemas,
            'SCHEMA_NAME'
        );
        console.log('this.record.LOAD_STRATEGY; ', this.record.LOAD_STRATEGY);
        if (
            this.record.LOAD_STRATEGY !== 'FLAT' ||
            !this.factSchemaNames.length
        ) {
            this.editLoadControlForm.controls.FACT_TABLE_NAME.patchValue('');
            this.factTablesNames = [];
        }
    }

    factSchemaSelected() {
        const factTables = this.factSchemaNamesBackup.filter(
            i =>
                i.SCHEMA_NAME ===
                    this.editLoadControlForm.value.FACT_SCHEMA_NAME &&
                i.ENV_NAME === this.editLoadControlForm.value.FACT_ENV_NAME
        );
        this.factTablesNames = this.removeDuplicates(factTables, 'TABLE_NAME');
        if (this.record.LOAD_STRATEGY !== 'FLAT') {
            this.editLoadControlForm.controls.FACT_TABLE_NAME.patchValue('');
        }
    }
    ...
    loadStrategyUpdated() {
        if (
            this.record.LOAD_STRATEGY !==
            this.editLoadControlForm.value.LOAD_STRATEGY
        ) {
            this.confirmationService.confirm({
                ...
                accept: () => {
                    this.editLoadControlForm.controls.ETL_STATUS.patchValue(
                        'HOLD'
                    );
                    ...
                    this.editLoadControlForm.controls.ETL_STATUS_REASON.patchValue(
                        `LOAD_STRATEGY is changed from ${this.record.LOAD_STRATEGY} to ${this.editLoadControlForm.value.LOAD_STRATEGY}`
                    );
                },
                reject: () => {
                    this.editLoadControlForm.controls.LOAD_STRATEGY.patchValue(
                        this.record.LOAD_STRATEGY
                    );
                },
            });
            ...
            this.editLoadControlForm.controls.ETL_STATUS.patchValue(
                this.record.ETL_STATUS
            );
            this.editLoadControlForm.controls.ETL_STATUS_REASON.patchValue(
                this.record.ETL_STATUS_REASON
            );
        }
    }
    ...
    setLoadStrategyValidators() {
        ...
        const DB_TABLE_UPDATE_DATE_COLUMN = this.editLoadControlForm.get(
            'DB_TABLE_UPDATE_DATE_COLUMN'
        );
        const DB_TABLE_PK_COLUMNS = this.editLoadControlForm.get(
            'DB_TABLE_PK_COLUMNS'
        );
        const CHECK_INDEX_EXIST = this.editLoadControlForm.get(
            'CHECK_INDEX_EXIST'
        );
        const T1_MAX_LOAD_END_DATE = this.editLoadControlForm.get(
            'T1_MAX_LOAD_END_DATE'
        );
        const T1_CLUSTER_ID = this.editLoadControlForm.get('T1_CLUSTER_ID');
        const T2_INSERT_DIR_BATCH_SIZE = this.editLoadControlForm.get(
            'T2_INSERT_DIR_BATCH_SIZE'
        );
        const T2_INSERT_BATCH_FILE_SIZE_GB = this.editLoadControlForm.get(
            'T2_INSERT_BATCH_FILE_SIZE_GB'
        );
        const T1_BATCH_IN_DAYS = this.editLoadControlForm.get(
            'T1_BATCH_IN_DAYS'
        );
        const T1_SPARK_CONFIG_ID = this.editLoadControlForm.get(
            'T1_SPARK_CONFIG_ID'
        );
        ...
        const FACT_SCHEMA_NAME = this.editLoadControlForm.get(
            'FACT_SCHEMA_NAME'
        );
        const FACT_TABLE_NAME = this.editLoadControlForm.get('FACT_TABLE_NAME');

        this.editLoadControlForm
            .get('LOAD_STRATEGY')
            .valueChanges.subscribe(LOAD_STRATEGY => {
                // Default
                DB_ID.enable();
                DB_SCHEMA.enable();
                DB_TABLE.enable();
    ...
    etlStatusChanged() {
        if (this.editLoadControlForm.value.ETL_STATUS !== 'HOLD') {
            this.editLoadControlForm.controls.ETL_STATUS_REASON.patchValue('');
            this.editLoadControlForm.controls.ETL_STATUS_REASON.setValidators(
                null
            );
    ...
    onSubmit() {
        ...
        const formValues = Object.assign(
            {},
            this.editLoadControlForm.getRawValue()
        );
        ...
        for (const key in formValues) {
            ...
            const dataType =
                this.recordMeta[index] && this.recordMeta[index].DATA_TYPE;
            if (formValues[key]) {
        ...
        formValues.CHECK_INDEX_EXIST =
            formValues.CHECK_INDEX_EXIST === '1' ? true : false;
        formValues.T1_BROADCAST =
            formValues.T1_BROADCAST === '1' ? true : false;

        const body = {
            record: formValues,
        };
        this.loadControlService.updateRecord(body).subscribe(
            (data: any) => {
                setTimeout(() => {
        ...
        if (
            this.record.LOAD_STRATEGY !==
                this.editLoadControlForm.value.LOAD_STRATEGY ||
            this.record.EMAIL_GROUP !==
                this.editLoadControlForm.value.EMAIL_GROUP ||
            this.record.EMAIL_ALERTS !==
                this.editLoadControlForm.value.EMAIL_ALERTS
        ) {
    ...
    checkForLinks() {
        ...
        cols.forEach(item => {
            ...
            if (
                value.match(linkRegex1) ||
                value.match(linkRegex2) ||
                value.match(linkRegex3)
            ) {
    ...
    setTableSourceValidators() {
        ...
        const DB_SCHEMA = this.editLoadControlForm.get('DB_SCHEMA');
        const DB_TABLE = this.editLoadControlForm.get('DB_TABLE');
        const DB_TABLE_PK_COLUMNS_SCHEMA = this.editLoadControlForm.get(
            'DB_TABLE_PK_COLUMNS'
        );
        const DB_TABLE_UPDATE_DATE_COLUMN = this.editLoadControlForm.get(
            'DB_TABLE_UPDATE_DATE_COLUMN'
        ...
        const RAW_FACTORY_RETENTION_STRATEGY = this.editLoadControlForm.get(
            'RAW_FACTORY_RETENTION_STRATEGY'
        );
        const RAW_FACTORY_RETENTION_DAYS = this.editLoadControlForm.get(
            'RAW_FACTORY_RETENTION_DAYS'
        );
        const RAW_FACTORY_MAX_LANDING_DATE = this.editLoadControlForm.get(
            'RAW_FACTORY_MAX_LANDING_DATE'
        );
        const RAW_FACTORY_SYNC_STATUS = this.editLoadControlForm.get(
            'RAW_FACTORY_SYNC_STATUS'
        );
        const EMAIL_GROUP = this.editLoadControlForm.get('EMAIL_GROUP');

        this.editLoadControlForm
            .get('TABLE_SOURCE')
            .valueChanges.subscribe(TABLE_SOURCE => {
                if (TABLE_SOURCE === 'ORACLE') {
                    DB_ID.setValidators([Validators.required]);
                    DB_SCHEMA.setValidators([
                        Validators.required,
                        Validators.pattern(this.tableRegex),
                    ]);
                    DB_TABLE.setValidators([
                        Validators.required,
                        Validators.pattern(this.tableRegex),
                    ]);
                    DB_TABLE_PK_COLUMNS_SCHEMA.setValidators([
                        Validators.required,
                        Validators.pattern(this.tableColumnRegex),
                    ]);
                    DB_TABLE_UPDATE_DATE_COLUMN.setValidators([
                        Validators.required,
                        Validators.pattern(this.tableColumnRegex),
                    ]);
                ...
                if (TABLE_SOURCE === 'RAW_FACTORY') {
                    ...
                    RAW_FACTORY_PATH.setValidators([
                        Validators.required,
                        Validators.pattern(this.s3UrlPattern),
                    ]);
                    RAW_FACTORY_MAX_LANDING_DATE.setValidators([
                        Validators.required,
                    ]);
        ...
        this.editLoadControlForm
            .get('EMAIL_ALERTS')
            .valueChanges.subscribe(EMAIL_ALERTS => {
                if (EMAIL_ALERTS === 'Y') {
    ...
    setRetentionStrategyValidators() {
        this.editLoadControlForm
            .get('RAW_FACTORY_RETENTION_STRATEGY')
            .valueChanges.subscribe(RAW_FACTORY_RETENTION_STRATEGY => {
                const RAW_FACTORY_RETENTION_DAYS = this.editLoadControlForm.get(
                    'RAW_FACTORY_RETENTION_DAYS'
                );
                if (RAW_FACTORY_RETENTION_STRATEGY !== 'NONE') {
                    RAW_FACTORY_RETENTION_DAYS.setValidators([
                        Validators.min(1),
                    ]);
                    RAW_FACTORY_RETENTION_DAYS.enable();
                } else {
        ...
        this.editLoadControlForm
            .get('T1_RETENTION_STRATEGY')
            .valueChanges.subscribe(T1_RETENTION_STRATEGY => {
                const T1_RETENTION_DAYS = this.editLoadControlForm.get(
                    'T1_RETENTION_DAYS'
                );
        ...
        this.editLoadControlForm
            .get('T2_T3_RETENTION_STRATEGY')
            .valueChanges.subscribe(T2_T3_RETENTION_STRATEGY => {
                const T2_T3_RETENTION_DAYS = this.editLoadControlForm.get(
                    'T2_T3_RETENTION_DAYS'
                );
    ...
    autoGenerateS3Url(type) {
        if (type === 'RAW_FACTORY_PATH') {
            ...
            if (
                ...
                this.showToast(
                    'error',
                    'Please provide TABLE_NAME and SCHEMA_NAME'
                );
            }
        }
    }
