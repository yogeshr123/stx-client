...
export class AddEditDbendpointComponent implements OnInit, OnDestroy {
    ...
    submit() {
        ...
        const formValues = Object.assign({}, this.addEditForm.value);
        formValues.UPDATE_DATE = `${new Date()}`;
        const tempEndpoint = formValues;
        ...
        // if (this.routeInfo.isEditMode) {
        this.dbEndpointsService[functionToCall](body).subscribe(
            (data: any) => {
                if (data && !data.error) {
