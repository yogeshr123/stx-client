...
export class AddEditSparkconfigComponent implements OnInit, OnDestroy {
    ...
    onSparkConfigKeySelected(value, index, controlValue) {
        let tempConf = remove(
            ...
            (item: any) => item.CONFIG_KEY === value
        );
        if (tempConf.length > 0) {
            ...
            for (const key in formControls) {
                if (formControls.hasOwnProperty(key)) {
                    ...
                    if (key === 'CONFIG_KEY') {
                        this.getSparkConfigFormGroup(
                            index
                        ).controls.CONFIG_KEY.disable();
                    }
                }
            }
            if (controlValue) {
                this.getSparkConfigFormGroup(index).controls.VALUE.patchValue(
                    controlValue
                );
            }
            ...
            this.showToast(
                'error',
                `${value} not found in spark configuration properties`
            );
        }
    }
    ...
    getUserInfo() {
        if (
            ...
            this.addEditForm.controls.UPDATED_BY.patchValue(
                this.appState.loggedInUser.USER_NAME
            );
        }
    }
                ...
                for (const key in formControls) {
                    if (formControls.hasOwnProperty(key)) {
                        ...
                        if (key === 'SPARK_CONF') {
                            ...
                            const sparkConfig = JSON.parse(
                                this.selectedSparkConfig[key]
                            );
                            let index = 0;
                            for (const config in sparkConfig) {
                                if (sparkConfig.hasOwnProperty(config)) {
                                    this.addItem();
                                    console.log(sparkConfig[config] + config);
                                    this.onSparkConfigKeySelected(
                                        config,
                                        index,
                                        sparkConfig[config]
                                    );
    ...
    submit() {
        ...
        const formValues = Object.assign({}, this.addEditForm.getRawValue());
        ...
        // if (this.routeInfo.isEditMode) {
        this.sparkConfigService[functionToCall](body).subscribe(
            (data: any) => {
                if (data && !data.error) {
    ...
    loadSchemaNames(ENV_NAME: string) {
        this.loadControlService.getDistinctSchemaNames(ENV_NAME).subscribe(
            (data: any) => {
    ...
    loadTableNames(ENV_NAME: string, SCHEMA_NAME: string) {
        this.loadControlService
            .getDistinctTableNames(ENV_NAME, SCHEMA_NAME)
            .subscribe(
                (data: any) => {
                    if (data.data && data.data.length > 0) {
    ...
    loadSparkConfigProperties() {
        this.sparkConfigPropertiesService.getSparkConfigProperties().subscribe(
            (data: any) => {
                if (data.data && data.data.length > 0) {
                    this.sparkConfigProperties = data.data;
                    if (
                        this.routeInfo.isEditMode ||
                        this.routeInfo.isViewOnly
                    ) {
                        this.setFormValues();
                    }
                }
