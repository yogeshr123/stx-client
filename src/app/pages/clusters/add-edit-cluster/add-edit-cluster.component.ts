...
export class AddEditClusterComponent implements OnInit, OnDestroy {
    ...
    get f() {
        return this.addEditClusterForm.controls;
    }
    ...
    getUserInfo() {
        ...
        if (appState.loggedInUser && appState.loggedInUser.USER_NAME) {
            this.addEditClusterForm.controls.UPDATED_BY.patchValue(
    ...
    onSubmit() {
        ...
        const request = {
            cluster: this.addEditClusterForm.value,
            oldClusterInfo: '',
        };
        ...
        this.clustersService[functionToCall](request).subscribe(
            (resp: any) => {
                if (resp && !resp.error) {
    ...
    populateUrls() {
        if (this.addEditClusterForm.controls.MASTER_IP_ADD.valid) {
            this.addEditClusterForm.controls.LIVY_URL.patchValue(
                `http://${this.addEditClusterForm.value.MASTER_IP_ADD}:8998`
            );
            this.addEditClusterForm.controls.YARN_URL.patchValue(
                `http://${this.addEditClusterForm.value.MASTER_IP_ADD}:8088`
            );
            this.addEditClusterForm.controls.SPARK_URL.patchValue(
                `http://${this.addEditClusterForm.value.MASTER_IP_ADD}:18080`
            );
            this.addEditClusterForm.controls.GANGLIA_URL.patchValue(
                `http://${this.addEditClusterForm.value.MASTER_IP_ADD}/ganglia`
            );
        }
    }
